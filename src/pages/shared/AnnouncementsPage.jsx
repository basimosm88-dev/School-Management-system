...
const AnnouncementsPage = ({ role }) => {
  ...
  return (
    <PageLayout role={role} title="School Announcements">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Broadcasts</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Official updates and notices from the school administration.</p>
        </div>
            ...
            <div key={ann.id} className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:border-primary/20">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">{ann.date}</p>
                  </div>
                </div>
              ...
              {ann.attachment && (
                <div className="mt-4 flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 w-fit">
                  <span className="material-symbols-outlined text-[20px] text-primary">attachment</span>
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{ann.attachment}</span>
                  <button className="text-[10px] font-black text-primary uppercase ml-4 hover:underline">Download</button>
                </div>
              )}
            </div>
          ))
