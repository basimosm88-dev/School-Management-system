...
const ResultsPage = ({ role }) => {
 ...
 <p className="mt-4 text-slate-500 font-bold animate-pulse">Loading Results...</p>
 </div>
 </PageLayout>
 );
 }
 ...
 <h2 className="text-2xl font-semibold text-slate-900 dark:text-white tracking-tight">{t('results')}</h2>
 <p className="text-xs font-bold text-slate-500 mt-1">Select a class to view academic performance and rankings.</p>
 </div>
 ...
 <p className="text-xs font-bold text-slate-500 mb-6">{c.studentsCount || 0} Students registered</p>
 ...
 className="pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-primary/20 transition-all w-48"
 />
 </div>
 <button 
 onClick={handlePrintClass}
 className="px-4 py-2 bg-slate-900 dark:bg-slate-800 text-white rounded-xl text-xs font-semibold uppercase tracking-wider flex items-center gap-2 hover:bg-slate-800 dark:hover:bg-slate-700 transition-all"
 >
 ...
 <h1 className="text-2xl font-semibold mb-1">{currentClass?.name}</h1>
 <p className="text-white/70 text-xs font-bold uppercase tracking-widest">Academic Overview</p>
 </div>
 ...
 <td className="px-6 py-4">
 <p className="font-bold text-sm text-slate-800 dark:text-slate-200">{res.name}</p>
 <p className="text-xs text-slate-400 font-medium uppercase tracking-tighter">ID: {res.id}</p>
 </td>
 <td className="px-6 py-4 text-center font-bold text-sm text-slate-700 dark:text-slate-300">
 {res.displayTotal}
 </td>
 <td className="px-6 py-4 text-center">
 <span className="font-semibold text-sm text-primary">{res.displayAverage}%</span>
 </td>
 <td className="px-6 py-4 text-center">
 ...
 className="px-6 py-2 bg-primary text-white rounded-xl text-xs font-semibold uppercase tracking-widest flex items-center gap-2 hover:bg-blue-700 transition-all shadow-lg shadow-primary/20"
 >
 <span className="material-symbols-outlined text-xl">print</span>
 ...
 <div>
 <h2 className="text-2xl font-semibold text-slate-900 dark:text-white tracking-tight">My Academic Record</h2>
 <p className="text-xs font-bold text-slate-500 mt-1">Official breakdown of your performance for the current year.</p>
 </div>
 <button 
 onClick={() => handlePrintStudent(currentUser.id)}
 className="px-6 py-2 bg-slate-900 dark:bg-slate-800 text-white rounded-xl text-xs font-semibold uppercase tracking-widest flex items-center gap-2 hover:bg-slate-800 transition-all"
 >
 <span className="material-symbols-outlined text-xl">print</span>
 Print My Record
 </button>
 ...
 <div className="w-24 h-24 rounded-3xl bg-white/10 flex items-center justify-center text-4xl font-semibold border border-white/20 shadow-inner backdrop-blur-sm">
 {student?.name?.charAt(0)}
 </div>
 <div>
 <h1 className="text-3xl font-semibold mb-3 tracking-tight">{student?.name}</h1>
 <div className="flex flex-wrap gap-6 text-white/60 text-xs font-bold uppercase tracking-widest">
 <div className="flex items-center gap-2">
 <span className="material-symbols-outlined text-primary text-2xl">school</span>
 Class: {currentClass?.name}
 </div>
 <div className="flex items-center gap-2">
 <span className="material-symbols-outlined text-indigo-400 text-2xl">fingerprint</span>
 ...
 <h3 className="font-semibold text-slate-900 dark:text-white uppercase tracking-wider text-xs">Academic Subject Breakdown</h3>
 </div>
 <div className="overflow-x-auto">
 <table className="w-full text-left">
 <thead>
 <tr className="bg-slate-50 dark:bg-slate-800/50 text-xs font-semibold text-slate-400 uppercase tracking-widest">
 ...
 <tr>
 <td colSpan="6" className="py-20 text-center text-slate-400 italic font-medium">No grades published for this student yet.</td>
 </tr>
 ) : (
 Object.keys(report.results).map((subject, idx) => (
 <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
 <td className="px-6 py-5 font-semibold text-sm text-slate-700 dark:text-slate-300">{subject}</td>
 <td className="px-4 py-5 text-center text-sm font-bold text-slate-600 dark:text-slate-400">{report.results[subject]["Before Midterm"]}</td>
 <td className="px-4 py-5 text-center text-sm font-bold text-slate-600 dark:text-slate-400">{report.results[subject]["Midterm"]}</td>
 <td className="px-4 py-5 text-center text-sm font-bold text-slate-600 dark:text-slate-400">{report.results[subject]["After Midterm"]}</td>
 <td className="px-4 py-5 text-center text-sm font-bold text-slate-600 dark:text-slate-400">{report.results[subject]["Final"]}</td>
 <td className="px-6 py-5 text-right font-semibold text-sm text-primary bg-slate-50/30 dark:bg-slate-800/20">{report.results[subject].average}%</td>
 </tr>
 ))
 )}
 </tbody>
 ...
 <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Total Score</p>
 <p className="text-3xl font-semibold text-slate-900 dark:text-white">
 {Object.values(report.results).reduce((a, b) => a + parseFloat(b.average), 0).toFixed(1)}
 </p>
 </div>
 <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-center shadow-sm">
 <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Average %</p>
 <p className="text-3xl font-semibold text-primary">
 {(Object.values(report.results).reduce((a, b) => a + parseFloat(b.average), 0) / Object.keys(report.results).length || 0).toFixed(1)}%
 </p>
 </div>
 <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-center shadow-sm">
 <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Class Rank</p>
 <p className="text-3xl font-semibold text-amber-500">#{report.rank}</p>
 </div>
 ...
 <p className="text-xs font-semibold text-white/60 uppercase tracking-widest mb-2">Promotion Status</p>
 <p className="text-xl font-semibold">{report.promotion}</p>
 </div>
 </div>
 </div>
 </PageLayout>
