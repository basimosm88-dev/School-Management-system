...
const NotificationsPage = ({ role }) => {
  ...
  const filteredNotifications = notifications.filter(n => {
    ...
    if (filter === 'messages') return n.title.includes('Message');
    return true;
  }).reverse();
  ...
  return (
    <PageLayout role={role} title="Notifications Center">
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {['all', 'unread', 'academic', 'messages', 'events'].map(f => (
          <button 
            key={f}
            onClick={() => setFilter(f)}
          ...
          filteredNotifications.map(n => (
            ...
            >
              <div className={`w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 ${getTypeStyles(n.type)}`}>
                <span className="material-symbols-outlined text-[22px]">{getIcon(n.type)}</span>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                  ...
                  <span className="text-[10px] text-slate-400 font-medium">{new Date(n.timestamp).toLocaleString()}</span>
                </div>
                <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2 leading-relaxed">
                  {n.message}
                </p>
                {n.actionLink && (
                  <button className="mt-3 text-[11px] font-black text-primary uppercase tracking-wider flex items-center gap-1 group-hover:gap-2 transition-all">
                    View Details
