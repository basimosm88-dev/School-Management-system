...
const AdminAttendancePage = () => {
  ...
  return (
    <PageLayout role="admin" title="Attendance Control Center">
      <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* FILTERS & TOGGLE */}
        <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm transition-colors space-y-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl w-full md:w-auto">
              <button 
                ...
                className={`flex-1 md:w-48 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewType === 'session' ? 'bg-white dark:bg-slate-700 shadow-sm text-primary' : 'text-slate-400'}`}
              >Session Details</button>
              <button 
                onClick={() => setViewType('daily')}
                className={`flex-1 md:w-48 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewType === 'daily' ? 'bg-white dark:bg-slate-700 shadow-sm text-primary' : 'text-slate-400'}`}
            ...
            <div className="flex gap-2 w-full md:w-auto">
              <button 
                ...
                className="px-6 py-3 bg-slate-50 dark:bg-slate-800 text-slate-500 text-[10px] font-black uppercase rounded-lg hover:bg-slate-100 transition-all border border-slate-100 dark:border-slate-700"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Class Filter</label>
              <select 
            ...
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Session Status</label>
              <select 
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm overflow-hidden transition-colors">
            ...
            <div className="overflow-x-auto max-h-[600px]">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] text-slate-400 font-black uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">
                  {viewType === 'session' ? (
                    <tr>
                    ...
                    filteredAttendance.map(a => (
                      <tr key={a.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        <td className="px-8 py-5">
                          <span className="font-black text-slate-700 dark:text-slate-200 block truncate max-w-[150px]">
                            {students.find(s => s.id === a.studentId)?.name || 'Unknown'}
                          </span>
                        </td>
                        <td className="px-8 py-5">
                          <div>
                            <span className="text-[11px] font-black text-slate-800 dark:text-slate-100 block uppercase tracking-tight mb-1">{a.subjectName}</span>
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{formatTime(a.startTime)} - {formatTime(a.endTime)}</span>
                          </div>
                        </td>
                        <td className="px-8 py-5 text-xs font-mono font-bold text-slate-400">{a.date}</td>
                        <td className="px-8 py-5">
                        ...
                        <td className="px-8 py-5 text-[10px] text-slate-400 font-bold uppercase truncate max-w-[120px]">
                    ...
                    dailySummaries.map(s => (
                      <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="px-8 py-5 text-center">
                          <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-md text-[10px] font-black text-slate-500">{s.sessionCount}</span>
                        </td>
                        <td className="px-8 py-5 text-xs font-mono font-bold text-slate-400">{s.date}</td>
                        <td className="px-8 py-5">
                          <span className={`px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.15em] border ${
                    ...
                    <tr>
                      <td colSpan="5" className="px-8 py-24 text-center">
                        <div className="flex flex-col items-center opacity-20">
                          ...
                          <p className="font-black uppercase tracking-[0.2em] text-xs">No records matching filters</p>
                        </div>
                      </td>
                    </tr>
                  )}
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm transition-colors">
              <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                  <h4 className="text-[11px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Attendance Rankings</h4>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Based on daily averages</p>
                </div>
              </div>
                  ...
                  <div key={s.id} className="group relative">
                    <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100/50 dark:border-slate-700/50 hover:border-primary/30 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <span className="text-xs font-black text-slate-800 dark:text-slate-200 block leading-none mb-1 line-clamp-1">{s.name}</span>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] text-slate-400 font-black">{s.rate}%</span>
                          </div>
                        </div>
                      </div>
                      <span className={`text-[10px] font-black ${s.rate > 90 ? 'text-emerald-500' : s.rate < 75 ? 'text-rose-500' : 'text-amber-500'}`}>
