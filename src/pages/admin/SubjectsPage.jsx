...
const SubjectsPage = () => {
 ...
 <h2 className="text-xl font-semibold text-slate-900 dark:text-white tracking-tight">{t('subjects')}</h2>
 <p className="text-xs font-bold text-slate-500 mt-1">{t('subjectsSubtitle')}</p>
 </div>
 ...
 {(sub.levels || []).map(lvl => (
 <span key={lvl} className="px-2 py-0.5 rounded-md text-xs font-semibold tracking-tighter bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800/50">
 {lvl}
 </span>
 ))}
 </div>
 ...
 <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{sub.weeklyPeriods}</span>
 <span className="text-xs text-slate-400 font-bold ">p/week</span>
 </div>
 </td>
 <td className="px-6 py-4">
...
const SubjectForm = ({ subject, onClose, onSave }) => {
 ...
 <div>
 <label className="text-xs font-bold text-slate-500 mb-3 block ">Assigned Levels</label>
 ...
 <button type="submit" className="px-8 py-2.5 bg-primary text-white text-sm font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-blue-700 transition-all transform active:scale-95">{subject ? 'Update Subject' : 'Save Subject'}</button>
 </div>
 </form>
 </div>
 );
