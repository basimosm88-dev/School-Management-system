...
const AdminTimetablePage = () => {
                      ...
                      {sessions.map(slot => {
                        ...
                        return (
                          <div key={slot.id} className={`w-full rounded-xl p-4 relative group transition-all border shadow-sm ${colorClass}`}>
                            ...
                            {slot.isBreak ? (
                              <p className="text-xs font-black uppercase tracking-tighter opacity-70">Break Session</p>
                            ) : (
  ...
  return (
    <PageLayout role="admin" title="Class Timetables">
      <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm transition-colors">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Select Class to Manage</label>
              <button 
                disabled={!selectedClassId}
                onClick={() => setShowModal(true)}
                className="px-6 py-3 bg-primary text-white text-[9px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20 hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-30 flex items-center gap-2"
              >
        ...
        {!selectedClassId ? (
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-700/50 h-[400px] flex flex-col items-center justify-center text-slate-300 transition-colors">
            ...
            <p className="font-black uppercase tracking-widest text-[10px] opacity-60">Choose a class to manage its schedule</p>
          </div>
        ) : (
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="px-8 py-8 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                <h3 className="text-xs font-black text-slate-800 dark:text-slate-200 uppercase tracking-[0.2em]">New Session Slot</h3>
              ...
              <div className="p-8 space-y-6">
                {error && (
                  <div className="p-4 bg-rose-50 text-rose-600 text-[10px] font-black rounded-2xl border border-rose-100 flex items-center gap-3 animate-shake">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Select Day</label>
                    <select 
                      value={formData.day}
                  ...
                  <div>
                    ...
                    <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                      <button 
                        ...
                        className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase transition-all ${!formData.isBreak ? 'bg-white dark:bg-slate-700 shadow-sm text-primary' : 'text-slate-400'}`}
                      >Subject</button>
                      <button 
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="form-input-custom w-full"
                    />
                    <p className="text-[8px] font-bold text-primary mt-1 uppercase tracking-tighter">* Manual Override Allowed</p>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full py-4 bg-primary text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-primary/20 hover:bg-blue-700 transition-all active:scale-[0.98]"
                >
                  Create Session
                </button>
              </div>
