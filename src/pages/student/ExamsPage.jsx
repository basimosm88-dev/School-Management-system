    ...
    <PageLayout role="student" title="Academic Results">
      <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="md:col-span-2 bg-primary p-10 rounded-2xl shadow-xl shadow-primary/20 flex flex-col justify-between relative overflow-hidden">
            <div className="relative z-10">
              <p className="text-white/60 text-[10px] font-black uppercase tracking-widest mb-2">Annual Academic Standing</p>
              <h3 className="text-white text-4xl font-black">{data.promotion}</h3>
            </div>
            <div className="mt-8 flex items-center gap-6 relative z-10">
              <div className="px-6 py-3 bg-white/10 backdrop-blur-md rounded-xl border border-white/20">
                <p className="text-white/40 text-[8px] font-black uppercase mb-1">Class Rank</p>
                <p className="text-white text-xl font-black">#{data.rank}</p>
              </div>
              <button 
                onClick={() => window.open(`/print-report/${currentUser?.id}`, '_blank')}
                className="px-8 py-3 bg-white text-primary text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 transition-all flex items-center gap-2"
              >
                <span className="material-symbols-outlined text-[18px]">print</span>
                Download Report
              </button>
            </div>
            <span className="material-symbols-outlined text-white/5 text-[200px] absolute -right-10 -bottom-10 rotate-12">workspace_premium</span>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center">
            ...
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Total Average</p>
            <h4 className="text-slate-900 dark:text-white text-3xl font-black">
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center">
            ...
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Total Subjects</p>
            <h4 className="text-slate-900 dark:text-white text-3xl font-black">{Object.keys(data.results).length}</h4>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm overflow-hidden transition-colors">
            <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20">
              <h3 className="text-xs font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Academic Breakdown</h3>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-white dark:bg-slate-900 text-slate-400 font-black uppercase text-[9px] tracking-[0.2em] border-b border-slate-100 dark:border-slate-800">
                  <tr>
                  ...
                  {Object.keys(data.results).length === 0 ? (
                    <tr>
                      <td colSpan="6" className="px-8 py-20 text-center text-slate-400">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest">No published results available yet</p>
                      </td>
                    </tr>
                  ) : (
                    Object.keys(data.results).map(subject => (
                      <tr key={subject} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        <td className="px-8 py-6">
                          <span className="font-black text-slate-800 dark:text-slate-200 uppercase text-xs">{subject}</span>
                        </td>
                        {examTypes.map(t => (
                        ...
                        <td className="px-8 py-6 text-right">
                          <span className={`px-4 py-2 rounded-lg text-[11px] font-black shadow-sm ${
        ...
        <div className="bg-amber-50 dark:bg-amber-900/10 p-8 rounded-2xl border border-amber-100 dark:border-amber-900/20 flex items-start gap-6">
          ...
          <div>
            <h5 className="font-black text-amber-900 dark:text-amber-100 text-xs uppercase tracking-widest mb-1">Official Academic Notice</h5>
            <p className="text-[10px] text-amber-800/70 dark:text-amber-200/70 font-bold leading-relaxed max-w-2xl">
