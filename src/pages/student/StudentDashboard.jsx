...
const StudentDashboard = () => {
 ...
 <h1 className="text-heading mb-1">{t(greetingKey)}, {currentUser.name}!</h1>
 <p className="text-white/80 text-label">Here is your academic progress for the current semester.</p>
 ...
 <p className="text-label">Security Alert: You are using a default password. Please change it in settings.</p>
 </div>
 )}
 </div>
 ...
 <h2 className="text-section text-slate-900 dark:text-slate-100">My Released Grades</h2>
 <button 
 ...
 `}>
 <span className="text-section">{grade.grade}</span>
 </div>
 <div>
 <h4 className="text-slate-900 dark:text-slate-100 text-label">{grade.subject} - {grade.type}</h4>
 <p className="text-label text-slate-500/80 dark:text-slate-400/80">Released on {grade.releaseDate ? new Date(grade.releaseDate).toLocaleDateString() : 'N/A'}</p>
 </div>
 </div>
 </div>
 ))
 ...
 <section className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-700/50 p-6 shadow-sm transition-colors">
 <h2 className="text-section text-slate-900 dark:text-slate-100 mb-6">School Notices</h2>
 ...
 <h4 className={`text-label  ${ann.priority === 'urgent' ? 'text-rose-700 dark:text-rose-400' : 'text-slate-800 dark:text-slate-200'}`}>{ann.title}</h4>
 <p className="text-label text-slate-500/80 dark:text-slate-400/80 mt-1 line-clamp-2">{ann.content || ann.message || ''}</p>
 </div>
 ))
 )}
 
 <h3 className="text-label text-slate-400/80 mt-6">Upcoming Events</h3>
 ...
 <h4 className="text-label text-slate-900 dark:text-white">{event.title}</h4>
