...
const StudentTimetablePage = () => {
  ...
  // 2. Filter timetable for this class
          ...
          {hoursRange.map(hour => {
            ...
            return (
              <div key={hour} className="grid grid-cols-[80px_1fr_1fr_1fr_1fr_1fr] border-b border-slate-100 dark:border-slate-800 min-h-[130px]">
                ...
                {days.map(day => {
                  const sessions = classSchedule.filter(t => {
                      ...
                      {sessions.map(slot => {
                        ...
                        return (
                          <div key={slot.id} className={`w-full rounded-xl p-4 relative group transition-all border shadow-sm ${colorClass}`}>
                            <div className="text-[9px] font-black opacity-40 mb-2 uppercase tracking-widest flex justify-between">
                              <span>{formatSlotTime(slot.startTime)} - {formatSlotTime(slot.endTime)}</span>
                            </div>
                            
                            {slot.isBreak ? (
                              <p className="text-xs font-black uppercase tracking-tighter opacity-70 mt-2">Break Session</p>
                            ) : (
                              <div>
                                <div className="font-black text-xs leading-relaxed uppercase tracking-tight mb-2">
                                  {slot.subjectName}
                                </div>
      ...
      <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-primary/5 p-8 rounded-xl border border-primary/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1 block">Assigned Schedule</p>
              <h3 className="font-black text-slate-900 dark:text-slate-100 text-2xl">{studentClass?.name || 'Unassigned'}</h3>
            </div>
          </div>
          <div className="flex gap-4">
            <div className="px-8 py-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 text-center shadow-sm">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Weekly Periods</p>
              <p className="text-2xl font-black text-primary">{classSchedule.length}</p>
            </div>
          </div>
        </div>
        ...
        {!studentClass ? (
          <div className="p-12 bg-rose-50 dark:bg-rose-900/10 rounded-xl border border-rose-100 dark:border-rose-900/20 text-center">
            ...
            <h3 className="text-xl font-black text-rose-900 dark:text-rose-100 mb-2 uppercase tracking-widest">Unassigned Record</h3>
            <p className="text-xs text-rose-600/70 font-bold uppercase tracking-widest">Contact administration to update your class records</p>
          </div>
        ) : (
