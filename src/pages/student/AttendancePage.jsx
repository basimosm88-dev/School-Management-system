          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm transition-all flex flex-col items-center text-center">
            <div className="relative w-24 h-24 mb-4">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-xl font-black text-slate-900 dark:text-slate-100">{summary.rate}%</span>
              </div>
            </div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Attendance Rate</p>
          </div>
          ...
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50/30 dark:bg-slate-800/10">
            <div className="flex bg-slate-200/50 dark:bg-slate-800/50 p-1.5 rounded-xl">
              <button 
                ...
                className={`px-8 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'daily' ? 'bg-white dark:bg-slate-700 shadow-sm text-primary' : 'text-slate-400'}`}
              >Daily Summaries</button>
              <button 
                onClick={() => setActiveTab('session')}
                className={`px-8 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'session' ? 'bg-white dark:bg-slate-700 shadow-sm text-primary' : 'text-slate-400'}`}
              >Session Breakdown</button>
            </div>
            <div className="flex items-center gap-3">
              <span className="w-3 h-3 rounded-full bg-primary animate-pulse"></span>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Real-time Performance Data</span>
            </div>
          </div>

          <div className="overflow-x-auto min-h-[400px]">
            <table className="w-full text-left text-sm">
              <thead className="bg-white dark:bg-slate-900 text-[10px] text-slate-400 font-black uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">
                {activeTab === 'daily' ? (
                  <tr>
                ...
                {activeTab === 'daily' ? (
                      ...
                      <td className="px-10 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 flex flex-col items-center justify-center border border-slate-200/50 dark:border-slate-700/50">
                            <span className="text-[10px] font-black text-slate-400 uppercase">{new Date(day.date).toLocaleString('default', { month: 'short' })}</span>
                            <span className="text-base font-black text-slate-900 dark:text-slate-100">{new Date(day.date).getDate()}</span>
                          </div>
                          <div>
                            <span className="font-black text-slate-800 dark:text-slate-200 block leading-tight">
                              {new Date(day.date).toLocaleDateString('default', { weekday: 'long' })}
                            </span>
                            <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{day.date}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-10 py-6 text-center">
                        <span className={`px-5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.1em] border shadow-sm ${
                  ...
                  summary.sessionHistory.map(session => (
                    <tr key={session.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="px-10 py-6">
                        <div>
                          <span className="text-[11px] font-black text-slate-800 dark:text-slate-100 block uppercase tracking-tight mb-1">{session.subjectName}</span>
                          <span className="text-[9px] font-bold text-primary bg-primary/5 px-2 py-0.5 rounded uppercase">{classes.find(c => c.id === session.classId)?.name}</span>
                        </div>
                      </td>
                      <td className="px-10 py-6">
                      ...
                      <td className="px-10 py-6 text-xs font-bold text-slate-400">{session.date}</td>
                      <td className="px-10 py-6 text-right">
                  ...
                  <tr>
                    <td colSpan="4" className="px-10 py-32 text-center text-slate-300">
                      <div className="flex flex-col items-center opacity-20">
                        ...
                        <p className="font-black uppercase tracking-[0.2em] text-xs">No records available for display</p>
                      </div>
                    </td>
                  </tr>
                )}
