      ...
      <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm transition-colors">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            ...
            <div>
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Date</label>
              <input 
                type="date" 
                value={selectedDate}
                onChange={(e) => {
            ...
            <div>
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Select Session</label>
              <select 
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="w-full px-4 py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all disabled:opacity-50"
              >
                Mark All Present
              </button>
            </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm overflow-hidden transition-colors min-h-[400px]">
          {!selectedSessionId ? (
            <div className="flex flex-col items-center justify-center h-[400px] text-slate-400">
              ...
              <p className="font-bold uppercase tracking-widest text-xs opacity-60">Select class and session to take attendance</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {classStudents.map(student => (
                    <tr key={student.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-8 py-5">
                        <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg font-mono text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                          {student.id}
                        </span>
                      </td>
                      <td className="px-8 py-5">
        ...
        {selectedSessionId && classStudents.length > 0 && (
          <div className="flex justify-end sticky bottom-6 z-10">
            <button 
              onClick={handleSave}
              className="px-12 py-4 bg-primary text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-2xl shadow-primary/30 hover:bg-blue-700 transition-all active:scale-95 flex items-center gap-3"
            >
