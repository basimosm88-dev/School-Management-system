      ...
      <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm transition-colors">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            <div>
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Exam Type</label>
              <select 
            ...
            <div>
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Select Class</label>
              <select 
            ...
            <div>
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Subject</label>
              <select 
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="flex-1 px-4 py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all disabled:opacity-30"
              >
                Save Draft
              </button>
              <button 
                ...
                disabled={!selectedSubjectId || isLocked}
                className="flex-1 px-4 py-3.5 bg-primary text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20 hover:bg-blue-700 transition-all disabled:opacity-30"
              >
                Submit
              </button>
              {(selectedExamType === 'Before Midterm' || selectedExamType === 'After Midterm') && currentExamStatus === 'APPROVED' && (
                <button 
                  ...
                  className="flex-1 px-4 py-3.5 bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-emerald-500/20 hover:bg-emerald-600 transition-all"
                >
                  Release Results
                </button>
              )}
          ...
          {currentExamStatus && (
            <div className="mt-6 flex items-center gap-3">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Workflow Status:</span>
              ...
              {isLocked && (
                <p className="text-[10px] font-bold text-rose-500 uppercase tracking-tighter italic">Editing is locked for this exam.</p>
              )}
            </div>
          )}
          
          {successMessage && (
            <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-black rounded-xl border border-emerald-100 dark:border-emerald-800/50 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-700/50 shadow-sm overflow-hidden transition-colors min-h-[400px]">
          {!selectedSubjectId ? (
            <div className="flex flex-col items-center justify-center h-[400px] text-slate-400">
              ...
              <p className="font-bold uppercase tracking-widest text-xs opacity-60">Select filters to enter academic results</p>
            </div>
          ) : (
                    ...
                    <tr key={student.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <span className="font-black text-slate-800 dark:text-slate-200 block">{student.name}</span>
                            <span className="text-[10px] text-slate-400 font-bold uppercase">{student.email}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <input 
                          ...
                          disabled={isLocked}
                          className="w-full px-4 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-lg font-black text-center text-slate-800 dark:text-white focus:ring-2 focus:ring-primary/50 transition-all disabled:opacity-50"
                          placeholder="--"
                        />
                      </td>
                      <td className="px-8 py-5">
                        <input 
                          ...
                          disabled={isLocked}
                          className="w-full px-4 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-lg text-[11px] font-bold text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-primary/50 transition-all disabled:opacity-50"
