...
const Header = () => {
 ...
 <span className="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-slate-400 text-2xl">search</span>
 <input
 className="pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg text-sm w-48 lg:w-64 focus:ring-2 focus:ring-primary/10 focus:bg-white dark:focus:bg-slate-900 transition-all outline-none dark:text-slate-200"
 ...
 <p className="text-xs text-slate-500 font-bold">{res.type}</p>
 </div>
 </button>
 ))}
 </div>
 ...
 <span className="material-symbols-outlined text-xl lg:text-2xl">language</span>
 </div>
 </button>
 {langOpen && (
 ...
 {unreadCount > 0 && (
 <span className="absolute top-1 right-1 w-4 h-4 lg:w-5 lg:h-5 bg-rose-500 text-white text-xs lg:text-xs font-bold flex items-center justify-center rounded-full border-2 border-white dark:border-slate-900 shadow-sm animate-bounce">
 ...
 {unreadCount > 0 && (
 <span className="text-xs bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 px-2 py-0.5 rounded-full font-bold">
 {unreadCount} new
 </span>
 )}
 </div>
 ...
 <p className="text-xs text-slate-400 mt-1 font-medium">{new Date(n.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
 </div>
 </div>
 </div>
 ))
 ...
 <span className="material-symbols-outlined text-xl lg:text-2xl">{darkMode ? 'light_mode' : 'dark_mode'}</span>
 </div>
 </button>
 </div>
